...
| **vaked-optitron** | Python cron loop | cron daily 05:33 UTC, dispatch | `tools/optitron/` ([README](tools/optitron/README.md)) · skill [`.claude/skills/vaked-optitron/`](.claude/skills/vaked-optitron/SKILL.md) | `gpt-oss-120b:online` (crawl) · `qwen3-thinking` (verify) · `deepseek-v4-flash` (bench) | Abstain-by-default optimization crawler: surfaces **one** novel, proven, independently-confirmed compiler/allocator/zig/rust/vaked optimization through a fail-closed gate (≥2 independent sources + repo/ledger novelty + a *reproduced* micro-benchmark + confidence ≥0.80), then opens an `agent`-labelled issue (the `swe_af` trigger) + announces to Mastodon/Telegram. Hash-chained findings ledger; $4/run cap. |
