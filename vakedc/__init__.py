#!/usr/bin/env python3
"""vakedc — the first executable Vaked front-end (lexer + parser -> LPG).

Implements 0011 pipeline stages 1-2 (parse + resolve) over grammar v0.3, producing
a Labeled Property Graph with byte-exact provenance attached at node instantiation.
Standalone, Python-3-stdlib-only prototype (the production parser is Zig later).

See ``vakedc/README.md`` and
``docs/superpowers/specs/2026-06-10-vakedc-parser-prototype-design.md``.
"""
