...
def _diag_to_lsp(d: dict, uri: str) -> dict:
    ...
    # End column: use byteEnd - byteStart as char count heuristic
...
# ---------------------------------------------------------------------------
# Keyword data for completion / hover
# ---------------------------------------------------------------------------
...
class LspServer:
    ...
    def _publish_diagnostics(self, uri: str, source: str) -> None:
        ...
        if builtins is not None:
            try:
                ...
                # Surface lex/parse errors as diagnostics
                diags = [
                    {
                        "range": {
    ...
    def _handle_definition(self, req: dict) -> dict:
        ...
        # Try to find the definition span in the current file
        if word and source:
...
def _word_at(source: str, line_0: int, char_0: int) -> str:
    ...
    if char_0 >= len(line):
        return ""
    # Expand left and right while identifier chars
    lo = char_0
