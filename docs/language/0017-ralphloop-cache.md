...
> takes the pragmatic path instead: it implements the **cache mechanism**
> (content-addressed objects + hash-chained JSONL — see
> `vaked/examples/compiler/ralphloop-cache.vaked`) using existing primitives and
> files the two language gaps it hit as #114 (build-time `memory` variant) and
> #115 (sequential `pipeline`/`parallel` strategy). 0017 is the
> "should the cached loop be a first-class kind?" proposal those gaps motivate;
> 0018 is what runs today. Reconcile them before implementing a new kind.
