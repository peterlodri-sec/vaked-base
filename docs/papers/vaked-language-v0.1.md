...
**Nickel** (Hamdaoui / Tweag) and **CUE** (van Lohuizen) provide structural record typing with contract/refinement support. Vaked borrows the structural typing idea but diverges:
...
**Dhall** (Gonzalez) emphasizes **totality** (all evaluation terminates). Vaked shares this principle but adds capability-attenuation semantics, which Dhall does not have.
...
**Determinism:**
- 21/22 valid examples produce byte-identical artifacts on repeated runs.
...
4. **Determinism:** All valid examples (21/22) compile to byte-identical artifacts on repeated runs (100+ iterations), enabling reproducibility and auditing. (The one invalid example is a negative test case by design.)
...
```
Determinism Oracle: 21/22 valid examples (100 iterations each)
