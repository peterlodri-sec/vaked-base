...
3. **Performance:** Compilation is fast for typical declarations (sub-100ms, though small examples are dominated by a ~50–75ms interpreter-startup floor). For scaling, a controlled multi-point sweep on GitHub Actions (a deterministic synthetic flat architecture of N worker fibers, per-stage medians; harness `scripts/benchmark-scalability-sweep.py`, full analysis in `docs/evaluation/scalability-analysis-v0.1.md`) found that an earlier build was **super-linear in the `lower` stage** — an empirical log-log slope of ~1.7 rising toward 2.0, with N=100,000 exceeding a 40-minute CI budget. Root-causing isolated the cause to a single O(N²) pass: `enrich_graph` performed a per-declaration linear scan of all graph nodes (`_node_for_chain`). Replacing that scan with a one-pass index made the pass O(N); the **same sweep, re-run, confirms all three stages are now linear** (slope ≈1.0): `lower` at N=30,000 dropped from 90s to 20.6s (4.4×), and N=100,000 — which previously did not finish — now compiles in ~70s (`lower`), ~119s end-to-end. The fix is purely a lookup change: all golden and determinism oracles remain byte-identical. The "1M" figure is therefore a *projection from the measured linear slope* (~703s for `lower`), not a measured point.
