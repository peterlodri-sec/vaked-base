//! QuickJS embedded interpreter — Logic Sandbox for the Vaked swarm.
//! Agents generate mini JS programs → QuickJS executes → returns to Zig.
//! Hard isolation: only exposed tools are available. No JIT → deterministic.
//! GENESIS_SEAL: 7c242080

const std = @import("std");

/// QuickJS runtime handle (opaque — managed by C library)
...
// QuickJS C API — linked statically
...
const QJS = struct {
    ...
    /// Execute JS code, return result as Zig string.
...
/// Logic Sandbox — agents submit JS, QuickJS executes, result returned.
/// Isolation: no filesystem, no network, no process access.
/// Only the functions explicitly exposed via Zig FFI are available.
pub const LogicSandbox = struct {
    ...
    /// Execute agent-provided logic. Safe — sandboxed, no JIT, deterministic.
