...
pub const ArenaHeader=extern struct{magic:u32 align(64),msg_count:u16,active_hydrators:u8,active_verifiers:u8,active_synthesizers:u8,msgs:[16]ArenaMsg};
...
pub fn poll(self:*WorkerPool,id:u32)?[]const u8{if(id>=16)return null;const m=&self.arena.msgs[id];if(m.status==2)return m.result[0..m.result_len];return null;}
pub fn complete(self:*WorkerPool,id:u32,r:[]const u8)void{if(id>=16)return;var m=&self.arena.msgs[id];m.status=2;m.result_len=@intCast(@min(r.len,8191));@memcpy(m.result[0..m.result_len],r[0..m.result_len]);switch(m.kind){.hydrator=>{self.arena.active_hydrators-=1;},.verifier=>{self.arena.active_verifiers-=1;},.synthesizer=>{self.arena.active_synthesizers-=1;}}}
pub fn tuiPoll(arena:*ArenaHeader,w:anytype)!void{
  ...
  var j:usize=0;while(j<16):(j+=1){var m=&arena.msgs[j];if(m.status==2){
    switch(m.kind){
