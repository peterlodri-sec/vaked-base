...
fn load_context() -> RepoContext {
    ...
    // When the snapshot is used, timeline_md is left empty intentionally —
    // inject_context() already guards with `if !ctx.timeline_md.is_empty()`,
    // and the snapshot itself embeds the TIMELINE metadata line.
