...
- **Content:** a short **caveman-style** toot written by `openai/gpt-oss-120b`
  (deterministic fallback), capped at **470 chars**, with code-controlled
  hashtags (`#vaked #ralph #<track>`), the **decision id** `[track#N]`, and a
  **link** to the decision log. Summary only — **never the full body**. Posts
  with `language=en`; a security/low-confidence decision gets a content warning
  (`spoiler_text`).
- **Idempotent + retried:** one toot per decision id (`Idempotency-Key` + an
  `announced` ledger event). Picks the **oldest un-announced** decision in a
  small recent window, so a Mastodon outage retries decisions in order instead
  of skipping them. `_post_toot` retries up to 3× honoring rate limits
...
- **No-op** without `MASTODON_ACCESS_TOKEN`. Config: `MASTODON_BASE_URL`
  ...
  `--base-url` endpoint as `decide`.

### Daily recap (`ralph digest`)

Once per day (near `RALPH_DIGEST_HOUR`, default **23:00 UTC**) `ralph digest`
posts a **funny, clear recap** of the last 24h of repo activity (commits +
ralph decisions) + ratify stats + **3 R&D suggestion links** (arXiv searches for
adjacent concepts), generated by `gpt-oss-120b`. Self-gates via a dated `digest`
ledger event; URLs counted at Mastodon's 23-char rate so links fit. Best-effort:
a failure is a CI `::warning::`, never red (the per-decision announce is the
fail-fast path). `--dry-run` to preview.
