...
# .vaked file, not here.
{
  description = "session-drive — generated by Vaked";
  ...
  inputs = {
    ...
    # index openIssues — sources (unpinned; flake.lock records the resolved rev).
    # 0012 §4.2: each index source becomes a flake input.
    openIssues-src-vaked-base = { url = "github:peterlodri-sec/vaked-base"; flake = false; };
  };

  outputs = { self, nixpkgs, ... }@inputs:
    let
      # runtime session-drive — systems = ["x86_64-linux"]
    ...
    in
    {
      ...
      nixosModules.session-drive = import ./nixos/session-drive.nix {
      ...
      apps = forAllSystems (system:
        let pkgs = nixpkgs.legacyPackages.${system};
        in {
          # surface sessionView (mode = raylib) — launcher app.
          ...
          sessionView = {
            type = "app";
            program = "${pkgs.writeShellScript "sessionView-launcher-deferred" ''
