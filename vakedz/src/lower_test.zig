...
test "lower: operator-field selects the not-yet-ported targets, and says so" {
    ...
    const result = try lowerSource(a, operator_field_src, fixture_path);
    // This fixture declares a fiber, an index emitting catalog.jsonl, and a
    // `parallel ... supervisor = otp` -- so exactly these three emitters are
    // selected but unported in this slice. When a follow-up slice ports one,
    // this list shrinks; it must NEVER silently grow.
    const expected = [_][]const u8{ "zig.daemoncfg", "catalog.jsonl", "otp.supervision" };
