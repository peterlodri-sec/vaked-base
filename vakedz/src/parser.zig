...
pub const MemberDecl = struct { name: []const u8 };
...
pub const Parser = struct {
    ...
    fn stmt(self: *Parser) error{ Parse, OutOfMemory }!Stmt {
        ...
        if (self.isIdent("member") and self.laMember()) return .{ .member = try self.memberDecl() };
    ...
    fn memberDecl(self: *Parser) !MemberDecl {
        self.i += 1;
        const id = try self.expectIdent();
